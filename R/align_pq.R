...
#' trimming), so phylopq never does it for you. Feed the returned alignment to
#' [MiscMetabar::build_phytree_pq()], [delim_pq()] or
#' [Biostrings::writeXStringSet()] instead.
#'
...
#'   [is_mafft_installed()]: first the `phylopq.mafftpath` option, then the
#'   system `PATH`.
...
#' MAFFT is an external program and is not installed by phylopq. It is
...
#' `options(phylopq.mafftpath = "/path/to/mafft")`.
#'
#' @author Adrien Taudière
#'
#' @seealso [is_mafft_installed()], [delim_pq()], [delim_multi_pq()],
#'   [MiscMetabar::build_phytree_pq()]
#'
#' @examples
#' \donttest{
#' library(MiscMetabar)
...
#'   case MAFFT is looked up in two places, in order: the `phylopq.mafftpath`
#'   option, then the system `PATH`.
#'
...
#' @author Adrien Taudière
#'
#' @seealso [align_pq()], [is_delim_installed()]
#'
#' @examples
#' is_mafft_installed()
#' @export
...
#'
#' Resolution order, mirroring `find_delim_exe()`: the `phylopq.mafftpath`
#' option, then the system `PATH`. MAFFT is a system package rather than a
#' program phylopq compiles, so there is no user-data-directory step.
#'
#' @return A length-one character path, or `""` when nothing was found.
#' @noRd
#' @keywords internal
find_mafft <- function() {
  opt <- getOption("phylopq.mafftpath")
...
as_dna_stringset <- function(x) {
  if (!requireNamespace("Biostrings", quietly = TRUE)) {
    cli::cli_abort(
      "Package {.pkg Biostrings} is required by {.fn align_pq}."
    )
  }
  ...
  if (!nzchar(exec) || !file.exists(exec)) {
    cli::cli_abort(c(
      ...
      "i" = "Give its path with {.arg exec}, or set it once with
             {.code options(phylopq.mafftpath = \"/path/to/mafft\")}.",
